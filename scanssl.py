...
class doc:
  ...
  def to_csv(self, delim=','):
    print self.doc_id + delim + self.title + delim + self.authors + delim + self.source
...
for i in xrange(9999):
  ...
  auts = chunks.last.split('Author(s)</td>')[0].split('<td bgcolor="#E6F0FF">')[0].split('</td>')[0]
  src = chunks.last.split('Source</td>')[0].split('<td bgcolor="#E6F0FF">')[0].split('</td>')[0]
  article = doc(i, title, auts, src)
  print article.to_csv()
  articles << article
  # Check if this article already has a valid link/ source
  #if chunks.last.find('[Article not available]') >= 0:
